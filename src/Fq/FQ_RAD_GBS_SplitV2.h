...
int  print_Ausage_RG()
{
	cout <<""
		"\n"
		"\tUsage: splitpool -i In1.fq In2.fq  -s <sample.info> -f <ferment.seq>\n"
		"\n"
		"\t\t-i    <str>   input PE FASTQ. For SE FASTQ\n"
		"\t\t-s    <str>   file with sample and barcodes, formatted as [sample_ID  barcode]\n"
		"\t\t-f    <str>   file with enzyme sequence, for example EcoRI is : AATTC\n"
		"\n"
		"\t\t-o    <str>   Output Dir for Split Files[PWD]\n"
		"\t\t-m            allow one mis-match on the sample barcode\n"
		"\t\t-c            allow one sample with multiple barcodes\n"
		"\n"
...
void More_HelpFQ5()
{
	cout<<"\n\
\n\
\t\t1.  splitpool -i <1.fq> <2.fq> -s <sample.info> -f <ferment.seq> -c -m -o ./\n\
\t\tsplitpool -i <1.fq>  -s <sample.info> -f <ferment.seq> -c -m -o ./\n\
\t\tthis will split the input pair end FASTQ to each sample by the sample information and enzyme sequence and output the results in current directory. \n\
...
{
	if (argc <=1 ) {print_Ausage_RG();return 0;}
