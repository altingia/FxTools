...
int  print_AusageFABP()
{
	cout <<""
		...
		"\t\t-l     <str>   input the relation table between the current FASTA and original scaffolds\n"
		"\t\t-o     <str>   outPut file\n"
		"\n"
...
\t\tThis will covert the positions of SNPs in current chromosomes back to scaffolds and output the result to a compressed file named AAA in current directory.\n\
\t\t(1.1)	The <in.snp> contains positions of all SNPs would like to locate on scaffolds. For example, if we want to locate two SNPs, which are now in base 100 on chr1 and base 200 on chr2 respectively, the input file would format in 2 columns:\n\
\t\tchr1  100\n\
\t\tchr2  200\n\
\t\t(1.2)	This < Ref.fa.merlist > show the relationship between current chromosomes and original scaffolds and formatted in 6 columns:\n\
chromosome_id  chromosome_start  chromosome_end  scaffold_id  scaffold_start  scaffold_end\n\
\n";
}
