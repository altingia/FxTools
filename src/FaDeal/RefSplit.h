...
int  print_RefSplit()
{
	cout <<""
		...
		"\t\t-i    <str>   input FASTA \n"
		"\n"
		"\t\t-o    <str>   output directory for splitting files, default [PWD]\n"
		"\t\t-s    <int>   fixed number of sequence in each splitting files, default [1]\n"
		"\t\t-f    <int>   fixed number of splitting files\n"
		"\n"
...
\n\
\t\t1.  split -i <in.fa> -o ./\n\
\t\tthis will create a folder named fa_cut in current directory, split the FASTA by sequence ID and output the files in this folder. The splitting files are in compressed format, please add the option -g if you would like unzip splitting files.\n\
\n\
\t\t2.  split -i <in.fa> -s X -o ./\n\
\t\tthis will create a folder named fa_cut in current directory, split the FASTA and output the files in this folder, while each file contains X sequence(s). The splittingfiles are in compressed format.\n\
\n\
\t\t3.  split -i <in.fa> -f X -o ./\n\
\t\tthis will create a folder named fa_cut in current directory, split the FASTA equally into X files and output the files in this folder. The splitting files are in compressed format.\n\
\n\
\n";
...
{
	if (argc <=1 ) {print_RefSplit();return 0;}
